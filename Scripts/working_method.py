...
def parse_data(input):
    ...
    x = np.loadtxt(input,delimiter=',', skiprows=1) #f
    vertaccel = np.diag(np.dot(x[:,1:3],x[:,4:6].T)) #f
    print "==== COMPLETE ====\n"
...
def modify_data():
	...
	header = "time,acc_x,acc_y,acc_z,gravity_x,gravity_y,gravity_z,vertaccel,coor\n"
	out.write(header)
	...
	if DEBUG:
	    ...
	    output += str(vertaccel[index])	#f
	    output += '\n'
	    out.write(output)

    print "==== COMPLETE ====\n"

def opt_distance():	#TEMP NAME

    global acc_y
    corr = []
    for lag in xrange(0,450):
	corr.append(coorelation(acc_y,offset=250,winsize=100,lag=lag))
    ...
    local_max(corr,base=False,filename="distance.csv")

def coorelation(data, offset=0, winsize=2, lag=0):
...
def local_max(data, base=True, filename="local_max.csv"):
	...
	if base and  max_y == curr and curr > average+1:
	    peaks.append(curr)
	    count+=1
	elif not base and max_y == curr:
	    peaks.append(curr)
	    count +=1
	else:
...
def normalize(marks):
	...
	#normalize height
	height = max(y[begin],y[mid],y[end])
...
def print_cycles(cycles):
    ...
    out = open("Debug/normalized.csv", 'w')
...
def build_avg_cycle(cycles):
	...
	var_z = 0.0
	for index in range(len(cycles)):
	    var_x += pow((cycles[index][i][0] - avg_x),2)
	    var_y += pow((cycles[index][i][1] - avg_y),2)
	    var_z += pow((cycles[index][i][2] - avg_z),2)
	
	dev_x = sqrt(var_x / len(cycles))
	dev_y = sqrt(var_y / len(cycles))
	dev_z = sqrt(var_z / len(cycles))

	points = [avg_x, avg_y, avg_z, dev_x, dev_y, dev_z]
...
def render_decision(probe1,probe2,template):
	...
	for line in reader:
	    ...
	    SD_x.append(float(line[4]))
	    SD_y.append(float(line[5]))
	    SD_z.append(float(line[6]))


    ## determine match ## 
    score1 = match(probe1, gallary_y, SD_y)
    score2 = match(probe2, gallary_y, SD_y)
